...
- **GitHub:** https://github.com/FayroSyrup/dev-stack-builder-website-PH-A5
...
The `key` helps React identify each item in the list. So when something changes, React can understand which item was changed, added or removed. That's why we need a unique key for every item.
...
```tsx
{stack.length === 0 ? <EmptyMessage /> : <StackList />}
```
...
To send something back from child to parent, we pass a function from the parent as a prop. Then the child calls that function and sends the data back. So simply — parent to child = props, child to parent = callback function through props.
